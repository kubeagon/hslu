# Exam: Basic Network Connectivity and Communications

> [!NOTE]
> These answers pass the exam with 94%!

## Question 1

![alt text](question01.png)

### Answer

He would use an **intranet** only accessible to employees of the same organisation. Extranets are for giving access to individuals from a different organisation or not part of your organisation.

## Question 2

![alt text](question02.png)

### Answer

- A → DSL
- B → satellite
- C → cable
- D → dialup telephone

## Question 3

![alt text](question03.png)

### Answer

Powerline networking lets you send data over powercables already installed in the walls by using special powerline adapters. 
The correct answer is a devide that connects to an existing home LAN using an adapter and an existing electrical outlet.

## Question 4

![alt text](question04.png)

### Answer

- The configuration that is actively running on the device is stored in RAM
- The contents of RAM are lost during a power cycle

## Question 5

![alt text](question05.png)

### Answer

SSH provides security to remote sessions by encrypting messages and using user authentication. Telnet is considered insecure and sends messages in plaintext.

## Question 6

![alt text](question06.png)

### Answer

(config)# service password-encryption

## Question 7

![alt text](question07.png)

### Answer

- Console interface
- VTY interface
- Privileged EXEC mode

## Question 8

![alt text](question08.png)

### Answer

This command prevents someone from viewing the running the configuration passwords.

## Question 9

![alt text](question09.png)

### Answer

letmein

## Question 10

![alt text](question10.png)

### Answer

to interrupt the ping process

## Question 11

![alt text](question11.png)

### Answer

- It provides a means to remotely manage a switch
- It is not associated with any physical interface on a switch
- It is associated with VLAN1 by default

## Question 12

![alt text](question12.png)

### Answer

Exit

## Question 13

![alt text](question13.png)

### Answer

It completes the remainder of a partially typed word in a command

## Question 14

![alt text](question14.png)

### Answer

The switch virtual interface

## Question 15

![alt text](question15.png)

### Answer

Issue the reload command without saving the running configuration

## Question 16

![alt text](question16.png)

### Answer

- RM-3-Switch-2A4
- SwBranch799

## Question 17

![alt text](question17.png)

### Answer

Segements are sent from the transport layer to the internet layer

## Question 18

![alt text](question18.png)

### Answer

A client host and a server running different operating systems can successfully exchange data

## Question 19

![alt text](question19.png)

### Answer

Ethernet, IP, TCP, HTTP

## Question 20

![alt text](question20.png)

### Answer

Segment

## Question 21

![alt text](question21.png)

### Answer

Frame

## Question 22

![alt text](question22.png)

### Answer

They define how messages are exchanged between the source and the destination

## Question 23

![alt text](question23.png)

### Answer

- DHCP
- FTP
- DNS

## Question 24

![alt text](question24.png)

### Answer

Subnet mask

## Question 25

![alt text](question25.png)

### Answer

Response timout

## Question 26

![alt text](question26.png)

### Answer

Flow control

## Question 27

![alt text](question27.png)

### Answer

Encapsulation

## Question 28

![alt text](question28.png)

### Answer

The administrator must first enter privileged EXEC mode before issuing the command

## Question 30

![alt text](question30.png)

### Answer

To enable the switch to be managed remotely

## Question 31

![alt text](question31.png)

### Answer

A router

## Question 32

![alt text](question32.png)

### Answer

Cloud

## Question 33

![alt text](question33.png)

### Answer

Application layer