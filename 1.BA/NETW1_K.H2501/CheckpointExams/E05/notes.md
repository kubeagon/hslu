# Exam: Network Applikation Communications

> [!NOTE]
> These answers pass the exam with 88%!
...
The TCP control bits (also called TCP flags) indicate the state of the connection and are what drive the three-way handshake:
...
- Transport Layer provides reliability (TCP), retransmissions, error recovery
- Transport layer uses port numbers to keep conversations seperate
- Port numbers map data to the correct applications
...
- Checksum → UDP also has a checksum
- Source port → UDP also has a source port
- Destination port → UDP also has a destination port
